...
describe('Prisma income repository', () => {
	...
	it('should list all incomes', async () => {
		...
		await repository.create({ name: 'Salary', month: '2026-02', amount: 2000.0 })
		await repository.create({ name: 'Previous Balance', month: '2026-02', amount: 500.0 })
	...
	it('should find income by id', async () => {
		...
		await repository.create({ name, month, amount })
	...
	it('should not find income by name when there are no incomes', async () => {
		...
		const response = await repository.findByNameAndMonth('Salary', '2026-02')
	...
	it('should not find income by name and month when current incomes does not have the corresponding name', async () => {
		...
		const response = await repository.findByNameAndMonth('Bônus', '2026-02')
	...
	it('should not find income by name and month when current incomes does not have the corresponding month', async () => {
		...
		const response = await repository.findByNameAndMonth('Salary', '2026-03')
	...
	it('should list incomes by month', async () => {
		...
		await repository.create({ name: 'Salary', month: '2026-02', amount: 2000.0 })
		await repository.create({ name: 'Previous Balance', month: '2026-02', amount: 500.0 })
		await repository.create({ name: 'January Salary', month: '2026-01', amount: 2500.0 })
	...
	it('should update an existing income', async () => {
		...
		const income = new Income({ id, name, month, amount })
	...
	it('should create an income', async () => {
		...
		await repository.create({ name: 'Bônus', month: '2026-02', amount: 200.0 })
