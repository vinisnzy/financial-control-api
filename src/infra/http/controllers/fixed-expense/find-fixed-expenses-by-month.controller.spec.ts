...
describe('GET /fixed-expenses/month/:month', () => {
	...
	it('returns 200 for valid month and body is array with fixed expense data', async () => {
		const response = await app.inject({
			method: 'GET',
			url: '/fixed-expenses/month/2026-05',
		})
	...
	it('returns 400 when month is missing', async () => {
		...
		expect(body.instance).toBe('/fixed-expenses/month/')
	...
	it('returns 400 when month format is invalid', async () => {
		const response = await app.inject({
			method: 'GET',
			url: '/fixed-expenses/month/05-2026',
		})
		...
		expect(body.title).toBe('Validation Error')
		expect(body.status).toBe(400)
		expect(body.instance).toBe('/fixed-expenses/month/05-2026')
		expect(body.errors).toEqual(expect.arrayContaining([expect.objectContaining({ field: 'month' })]))
	})
})
