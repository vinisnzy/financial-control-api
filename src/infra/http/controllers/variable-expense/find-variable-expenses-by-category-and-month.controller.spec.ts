...
vi.mock('@/main/app.js', () => ({
	container: {
		findVariableExpensesByCategoryAndMonth: {
			execute: vi
				.fn()
				.mockResolvedValue([
					{
...
describe('GET /variable-expenses/category/:category/month/:month', () => {
	...
	it('returns 200 for valid category and month', async () => {
		const response = await app.inject({
			method: 'GET',
			url: '/variable-expenses/category/food/month/2026-05',
		})
	...
	it('returns 400 when month format is invalid', async () => {
		const response = await app.inject({
			method: 'GET',
			url: '/variable-expenses/category/food/month/05-2026',
		})
		...
		expect(body.instance).toBe('/variable-expenses/category/food/month/05-2026')
	...
	it('returns 400 when category is invalid', async () => {
		const response = await app.inject({
			method: 'GET',
			url: '/variable-expenses/category/invalid-category/month/2026-05',
		})
		...
		expect(body.title).toBe('Validation Error')
		expect(body.status).toBe(400)
		expect(body.instance).toBe('/variable-expenses/category/invalid-category/month/2026-05')
