...
vi.mock('@/main/container.js', () => ({
	container: {
		findNecessaryVariableExpensesByMonth: {
...
describe('GET /variable-expenses/necessary/month/:month', () => {
	...
	it('returns 200 for valid month and body is array with variable expense data', async () => {
		const response = await app.inject({
			method: 'GET',
			url: '/variable-expenses/necessary/month/2026-05',
		})
	...
	it('returns 400 when month format is invalid', async () => {
		const response = await app.inject({
			method: 'GET',
			url: '/variable-expenses/necessary/month/05-2026',
		})
		...
		expect(body.instance).toBe('/variable-expenses/necessary/month/05-2026')
