...
export class FindIncomeByNameAndMonthUseCase {
	...
	async execute(name: string, month: string, userId: string): Promise<FindIncomeByNameAndMonthResponse> {
		const income = await this.repository.findByNameAndMonth(name, month, userId)
		if (!income) {
